'use client'

import { Dialog, Transition, DialogPanel, TransitionChild, DialogTitle } from '@headlessui/react'
import { Fragment, useState } from 'react'
import { CircleDollarSign, CheckCircle, X } from 'lucide-react'
import { Comprobante } from '@/app/dashboard/cajero/cobros/datos/comprobantes'

interface ConfirmModalPagoProps {
  isOpen: boolean
  onConfirm: () => Promise<void> | void
  onCancel: () => void
  comprobantes: Comprobante[]
}

export default function ConfirmModalPago({ isOpen, onConfirm, onCancel, comprobantes }: ConfirmModalPagoProps) {
  const [procesando, setProcesando] = useState(false)

  const totalGeneral = comprobantes.reduce((acc, c) => acc + c.totalPagar, 0)

  const handleConfirm = async () => {
    try {
      setProcesando(true)
      await onConfirm()
    } finally {
      setProcesando(false)
    }
  }

  return (
    <Transition appear show={isOpen} as={Fragment}>
      <Dialog as="div" className="relative z-[100]" onClose={onCancel}>
        <TransitionChild
          as={Fragment}
          enter="ease-out duration-200"
          enterFrom="opacity-0"
          enterTo="opacity-100"
          leave="ease-in duration-150"
          leaveFrom="opacity-100"
          leaveTo="opacity-0"
        >
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm" />
        </TransitionChild>

        <div className="fixed inset-0 flex items-center justify-center p-3 sm:p-4">
          <TransitionChild
            as={Fragment}
            enter="ease-out duration-300"
            enterFrom="opacity-0 scale-95 translate-y-3"
            enterTo="opacity-100 scale-100 translate-y-0"
            leave="ease-in duration-200"
            leaveFrom="opacity-100 scale-100 translate-y-0"
            leaveTo="opacity-0 scale-95 translate-y-3"
          >
            <DialogPanel className="bg-white rounded-2xl shadow-2xl w-full max-w-sm sm:max-w-md overflow-hidden border border-blue-100 max-h-[90vh] overflow-y-auto">
              
              {/* Icono superior */}
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 p-5 flex justify-center">
                <div className="relative">
                  <div className="absolute inset-0 bg-green-500 rounded-full animate-ping opacity-20" />
                  <div className="relative bg-gradient-to-br from-green-600 to-emerald-500 p-3 rounded-full shadow-lg">
                    <CircleDollarSign className="text-white" size={32} />
                  </div>
                </div>
              </div>

              {/* Contenido */}
              <div className="p-5">
                <DialogTitle className="text-lg sm:text-xl font-bold text-gray-800 text-center mb-3">
                  Confirmar pago de comprobantes
                </DialogTitle>

                <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm sm:text-base text-gray-700">
                  {comprobantes.length > 0 ? (
                    <>
                      <div className="border-b pb-2">
                        <span className="font-medium text-gray-600">
                          Total de comprobantes: {comprobantes.length}
                        </span>
                      </div>

                      <div className="max-h-40 overflow-y-auto space-y-1">
                        {comprobantes.map((c) => (
                          <div key={c.idComprobante} className="flex justify-between text-xs sm:text-sm">
                            <span className="text-gray-600">
                              #{c.idComprobante} — {c.lectura?.medidor?.ubicacionSocio?.direccion || 'Sin dirección'}
                            </span>
                            <span className="font-semibold text-blue-600">
                              {new Intl.NumberFormat('es-BO', {
                                style: 'currency',
                                currency: 'BOB',
                              }).format(c.totalPagar)}
                            </span>
                          </div>
                        ))}
                      </div>

                      <div className="border-t pt-3 text-lg font-bold text-blue-700 flex justify-between">
                        <span>Total a pagar:</span>
                        <span>
                          {new Intl.NumberFormat('es-BO', {
                            style: 'currency',
                            currency: 'BOB',
                          }).format(totalGeneral)}
                        </span>
                      </div>
                    </>
                  ) : (
                    <p className="text-center text-gray-500">No hay comprobantes seleccionados.</p>
                  )}
                </div>
              </div>

              {/* Botones */}
              <div className="bg-gray-50 px-5 py-3 flex flex-col-reverse sm:flex-row justify-center gap-3">
                <button
                  onClick={onCancel}
                  className="w-full sm:w-auto px-4 py-2 rounded-xl bg-white border-2 border-gray-300 text-gray-700 hover:bg-gray-100 active:scale-95 transition font-medium flex items-center justify-center gap-2"
                >
                  <X size={16} />
                  Cancelar
                </button>
                <button
                  onClick={handleConfirm}
                  disabled={procesando || comprobantes.length === 0}
                  className={`w-full sm:w-auto px-4 py-2 rounded-xl 
                  ${procesando || comprobantes.length === 0
                      ? 'bg-green-400 cursor-not-allowed'
                      : 'bg-gradient-to-r from-green-700 to-emerald-500 hover:from-green-600 hover:to-emerald-600'
                    } text-white active:scale-95 transition font-semibold flex items-center justify-center gap-2`}
                >
                  <CheckCircle size={16} />
                  {procesando ? 'Procesando...' : 'Confirmar pago'}
                </button>
              </div>
            </DialogPanel>
          </TransitionChild>
        </div>
      </Dialog>
    </Transition>
  )
}
